...
$('h4 a').on('click', function(e){
	...
	$('#oldform').hide();
	$('#content').load(url + ' #newform').delay(500).hide().fadeIn('slow');
...
$('tr a').on('click', function(e){
	...
	$('#wholepage*').remove();
	$('#contentTitle*').load(url + ' #wholeTopicTitle').hide().fadeIn('slow')
	$('#contentText*').load(url + ' #wholeTopicText').hide().fadeIn('slow');
});
